 ...
 *
 * Generates loadable ProjectState objects for each feasibility scenario (A1–A8).
 ...
 *
 * Fixture data comes from test/golden/feasibilityFixtures.ts (shared with CI tests).
 */
...
function buildFeasibilityDemo(scenario: FeasibilityDemoScenario): ProjectState {
  ...
  // Build sound streams — each pad gets a stream with a few test events
  const soundStreams: SoundStream[] = scenario.noteNumbers.map((noteNum, i) => {
    const events: SoundEvent[] = [];
    // Generate 4 evenly spaced events for each voice
    for (let t = 0; t < 4; t++) {
      events.push({
        startTime: t * 0.5,
        duration: 0.25,
